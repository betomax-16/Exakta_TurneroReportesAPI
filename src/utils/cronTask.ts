...
async function createReportsAndSendEmails(startDate: moment.Moment, selectedDate: moment.Moment) {
    ...
    for (let index = 0; index < HEADER_ROW.length; index++) {
        ...
        ws.cell(1, index + 1).string(column.value).style(style);
    }
    ...
    for (let fila = 0; fila < generalReport.length; fila++) {
        ...
        for (const property in row) {
            ws.cell(fila + 2, column).string(row[property].toString()).style(style);
            column++;
        }
    }
...
export function reportWeek() {
    ...
    const cronJob: CronJob = new CronJob('47 10 * * *', async () => {
        try {
